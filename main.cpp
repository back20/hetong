...
// ============================================================================
// 1. 异步后台工作引擎 (负责无损 Office XML/Word 拼接，防止界面卡死)
// ============================================================================
...
public slots:
    void doGenerateTask(const QJsonObject& formData) {
        ...
        // 模拟核心 C++ 引擎在后台高能拼装 Word 附页及表格
        // 在实际生产中，此处会调用 OpenXLSX 或 DuckX 库操作底层 XML
        QThread::msleep(2000); 

        bool success = true; // 假设 XML 句柄写入成功
        if (success) {
            emit taskFinished("自备吊车租赁合同（C++ 高能流 v2.0）已成功安全导出！\n随附台班法律附页已自动固化完成。");
        } else {
            emit taskError("致命错误：底层 Office XML 句柄写入失败，请检查文件是否被占用。");
        }
    }
...
public:
    MainWindow(QWidget *parent = nullptr) : QMainWindow(parent) {
        ...
        // 突出核心生成按钮
        btnGen->setStyleSheet("background-color: #2980b9; color: white; font-weight: bold; font-size: 13px;");
        ...
        connect(btnAutoCode, &QPushButton::clicked, this, [=]() {
            QString prefix = entryCode->text().trimmed();
            QString timestamp = QDateTime::currentDateTime().toString("yyyyMMddhhmm");
            entryCode->setText(prefix + timestamp);
            statusBar->showMessage(" 状态提示: 合同编号已基于高精度时钟自动生成。");
        });

        // 互换甲乙方主体
        connect(btnSwap, &QPushButton::clicked, this, [=]() {
            QString nameA = entryAName->text(), taxA = entryATax->text(), telA = entryATel->text(), bankA = entryABank->text(), accA = entryAAcc->text();
            entryAName->setText(entryBName->text()); entryATax->setText(entryBTax->text()); entryATel->setText(entryBTel->text()); entryABank->setText(entryBBank->text()); entryAAcc->setText(entryBAcc->text());
            entryBName->setText(nameA); entryBTax->setText(taxA); entryBTel->setText(telA); entryBBank->setText(bankA); entryBAcc->setText(accA);
            QMessageBox::information(this, "互换完成", "甲乙方商事主体及收付款行号已安全对调。");
        });

        // 触发生成任务
        connect(btnGen, &QPushButton::clicked, this, [=]() {
            if(entryCarNo->text().trimmed().isEmpty() || entryWorkAddr->text().trimmed().isEmpty()) {
                QMessageBox::warning(this, "校验失败", "【吊车车牌】与【施工地点】为核心法装配要素，不能为空！");
                return;
            }
            statusBar->showMessage(" 状态提示: 异步公文流引擎已启动，正在拼装生成 Office 合同...");
            
            QJsonObject formData;
            formData["code"] = entryCode->text();
            formData["car_no"] = entryCarNo->text();
            formData["work_addr"] = entryWorkAddr->text();
            formData["pay_day"] = entryPayDay->text();
            emit startTask(formData);
        });
...
private slots:
    void onGenerationSuccess(const QString& msg) {
        QMessageBox::information(this, "生成成功", msg);
        statusBar->setStyleSheet("color: green;");
        statusBar->showMessage(" 状态提示: 合同导出成功！");
    }

    void onGenerationError(const QString& err) {
        QMessageBox::critical(this, "合同构建中断", err);
        statusBar->setStyleSheet("color: red;");
        statusBar->showMessage(" 状态提示: 核心流写入异常。");
    }
...
#include "main.cpp.moc" // 确保元对象编译器(MOC)正常运作
