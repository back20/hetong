...
// ============================================================================
// 1. 异步后台工作引擎 (负责无损 Office 拼装，防止界面卡死)
// ============================================================================
...
public slots:
    void doGenerateTask(const QJsonObject& formData) {
        ...
        // 模拟核心 C++ 引擎在后台高能拼装
        QThread::msleep(2000); 
        emit taskFinished("自备吊车租赁合同（C++ 单文件高能流 v1.0）已成功安全生成！\n随附台班法律附页已自动固化完成。");
    }
signals:
...
public:
    MainWindow(QWidget *parent = nullptr) : QMainWindow(parent) {
        this->setWindowTitle("自备吊车租赁合同智能化管理工具 v1.0 (C++ Core)");
        this->resize(860, 720);
        ...
        connect(btnSwap, &QPushButton::clicked, this, [=]() {
            ...
            entryBName->setText(nameA); entryBBank->setText(bankA); entryBAcc->setText(accA);
...
private slots:
    void onGenerationSuccess(const QString& msg) { QMessageBox::information(this, "导出成功", msg); }
    void onGenerationError(const QString& err) { QMessageBox::critical(this, "中断", err); }
...
// 核心修复：单文件架构下，必须包含 main.moc，不能是 main.cpp.moc 
#include "main.moc"
